            ...
            Action::CreateSession {
                ...
                if let Some(ws) = self.tui.workspaces.get(workspace_index) {
                    ...
                    if let Some(ws_dir) = actual_dir {
                        ...
                        // 预创建 JSONL 文件，写入 session metadata + session_name，
                        // 确保重启后 populate_workspaces 可发现且 extract_session_name 能提取名称
                        if let Some(parent) = session_path.parent() {
                            let _ = std::fs::create_dir_all(parent);
                        }
                        let session_timestamp = iso8601_now();
                        ...
                        // 写入 session_name 行，使 extract_session_name 无需回退到 "New Session"
                        if !name.is_empty() {
                            if let Ok(name_line) = serde_json::to_string(&serde_json::json!({
                                "type": "session_name",
                                "name": name,
                            })) {
                                content.push('\n');
                                content.push_str(&name_line);
                            }
                        }
                        content.push('\n');
...
/// 生成当前时间的 ISO 8601 格式字符串（无 chrono 依赖）
/// 基于 Hinnant 的 civil date 算法，有效范围 1970-2100
...
mod tests {
    ...
    fn test_iso8601_now_format() {
        let ts = iso8601_now();
        // Should match ISO 8601 pattern: 2026-06-25T10:10:22.005Z
    ...
    #[test]
    fn test_iso8601_now_unique() {
        let ts1 = iso8601_now();
        let ts2 = iso8601_now();
        // Two calls in sequence should be close but different is OK
        // Just validate format for both
        assert!(ts1.len() >= 20);
        assert!(ts2.len() >= 20);
    }
}
